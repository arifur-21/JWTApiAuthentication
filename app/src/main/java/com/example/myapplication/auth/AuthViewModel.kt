...
class AuthViewModel @Inject constructor(private val userRepository: UserRepository) : ViewModel() {
    ...
    fun forgotPassword(userRequest: ForgotPass){
        viewModelScope.launch {
            userRepository.forgotPass(userRequest)
        }
    }
    ...
    fun validateCredentials(emailAddress: String, fname: String,lname: String, password: String,
     isLogin: Boolean) : Pair<Boolean, String> {

        var result = Pair(true, "")
        if(TextUtils.isEmpty(emailAddress) || (!isLogin && TextUtils.isEmpty(fname)) ||  (!isLogin && TextUtils.isEmpty(lname)) || TextUtils.isEmpty(password)){
